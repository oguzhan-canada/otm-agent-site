...
Commodity analysts routinely integrate satellite imagery, market microstructure data, and econometric models to form views on global oil storage. This workflow demands expertise spanning remote sensing, time-series analysis, and energy trading — a combination rarely found in a single practitioner and difficult to systematize. We present **OTM-Agent**, an LLM-orchestrated agentic system that wraps an operational three-stage oil tank monitoring pipeline — Sentinel-1 SAR processing, feature engineering, and signal generation — with a typed skill library (20 skills across 7 categories), a four-tier memory system (episodic, semantic, procedural, working), Bayesian persona vectors (64 dimensions), and a multi-objective reward function grounded in paper-trading profit and loss. The agent's planner selects skill sequences conditioned on retrieved memory and persona state, while a separate executor dispatches typed actions with tracked preconditions and side effects. We evaluate the system using an 8-benchmark suite purpose-built for commodity intelligence agents, with a 12-month out-of-sample holdout enforced by static AST analysis integrated into CI. Three pre-registered benchmarks were executed at planned scale with real agent calls and re-verified across two versions (v1.0 and v1.1): OOS Replay produced 54.4% directional accuracy (n=237, CI crosses 50% — statistically indistinguishable from random); Skill-Retention achieved 81.7% plan inclusion (5.2× corrected random baseline, stable across both versions); and Persona-Align confirmed a pre-registered null result (1.001× baseline, p=0.70 — in-prompt persona injection does not produce detectable behavioral change at v0.1). Two additional exploratory benchmarks were executed at reduced scale due to budget constraints: Mem-Utility (n=25, no detectable memory benefit, −0.064, CI crosses zero) and MultiTurn-Persona (n=7 sequences, +0.173 convergence delta, p=0.109 — statistically underpowered, not interpretable at this sample size). The exploratory results are reported for transparency but were not pre-registered and lack the statistical power for confirmatory inference; both are scoped for full-scale execution in v1.2. Six audit cycles identified and corrected measurement infrastructure bugs — OOS leakage in training queries, planner stop-condition errors, and incomplete cost tracking — with each fix documented, re-verified, and published transparently. A seventh cycle was applied to the audit process itself when two exploratory benchmarks were executed without pre-registration and presented with confirmatory-grade framing; the overclaim was caught by external review and corrected across all artifacts within 24 hours — seven total audit revolutions with full disclosure. Total evaluation cost: $20.51 across all v1.1 runs ($16.55 for the three pre-registered benchmarks, $3.96 for the two exploratory benchmarks — $0.51 over the committed $20 API credit cap due to cost variance during MultiTurn-Persona execution). On OOS-clean data, the regime classifier achieves 65.4% walk-forward accuracy against a 52.9% majority-class baseline, Stage 1 Kalman filtering aligns with EIA inventory direction at 52.8%, and the full system produces daily composite trading signals with quantified confidence. A strategy investigation identified the root cause of the initially negative backtest Sharpe (naive tightness-delta signal with no confidence gating) and implemented a regime-weighted ML composite with walk-forward training, confidence thresholding, and drawdown-based risk controls. We release the evaluation framework, dashboard, and holdout enforcement tooling as contributions to the broader agentic AI research community.
...
8. **An 8-benchmark evaluation suite** with domain-specific tests for long-context utilization, skill retention, persona alignment, distribution shift robustness, out-of-sample replay, memory utility, multi-turn persona consistency, and memory tier ablation. Three pre-registered benchmarks executed at planned scale and re-verified across two versions (v1.0, v1.1): 81.7% skill-inclusion accuracy (5.2× baseline), pre-registered persona null confirmed (p=0.70), directional accuracy statistically indistinguishable from random (54.4% full-year, CI crosses 50%). Two exploratory benchmarks executed at reduced scale without pre-registration: memory utility null (−0.064, n=25, CI crosses zero) and multi-turn persona convergence underpowered (+0.173, p=0.109, n=7 — not interpretable at this sample size). Six audit cycles documented with preserved trails. Total evaluation cost: $20.51.

9. **An audit-cycle methodology** applied to the evaluation infrastructure itself. Seven complete audit cycles — OOS leakage, model retrain provenance, skill-retention measurement bugs, planner stop-condition, cost tracking, v3.0.0 architecture verification, and process slip detection (the methodology applied to its own creator) — each discovered and corrected integrity failures with preserved audit trails and transparent disclosure. The methodology, crystallized in the operational rule "copy from source, never from memory," demonstrates that evaluation rigor should extend to the evaluation tooling, not just the system under test.
...
**v1.0 Result (post-fix, 2026-05-24).** Plan inclusion: **81.7%** (CI: 71.7%–91.7%, n=60). Top-3: 55.0%. Top-1: 21.7%. Mean plan length: 3.4 steps. 14/20 skills at 100% inclusion. Corrected random baseline: 15.7% — observed is **5.2× baseline**.
...
#### 8.6.4 OTM-Mem-Utility (Exploratory, n=25)
...
**Status: Exploratory.** This benchmark was not pre-registered. The runner was written and executed in the same session. The designed sample size (500 queries) was reduced to n=25 due to budget constraints. Results are reported for transparency but lack the statistical power for confirmatory inference.
...
**Interpretation.** The aggregate null is the only interpretable finding. Per-category results (n=5 each) have extremely wide CIs and are not statistically interpretable — they are reported for completeness but should not be used to draw directional conclusions about memory utility by domain. The v0.1 memory architecture does not produce detectable improvement in response quality on this benchmark; further investigation is needed to determine whether the issue is relevance scoring, memory content quality, query coverage, or measurement methodology. Full-scale execution at n≥200 is scoped for v1.2.

#### 8.6.5 OTM-MultiTurn-Persona (Exploratory, n=7)
...
**Status: Exploratory.** This benchmark was not pre-registered. The runner was written and executed in the same session. The designed sample size (100 sequences) was reduced to n=7 due to budget constraints. At n=7, the benchmark lacks statistical power to reliably distinguish signal from noise; the result is not interpretable as confirmatory evidence.

**Results (v1.1, n=7, seed=42, $2.57).** Convergence delta: **+0.173** (95% CI: −0.064 to +0.378). Mean early divergence: 0.162. Mean late divergence: 0.335. Wilcoxon p=0.1094 — not statistically significant at α=0.05.

**Interpretation.** At n=7 sequences, this result is statistically underpowered. The CI includes zero and the p-value exceeds conventional significance thresholds. The positive direction (+0.173) is noted but cannot be distinguished from noise at this sample size. No conclusions about persona convergence mechanics should be drawn from this run. A confirmatory run at n≥20 with pre-registration is scoped for v1.2.
...
The corrected total ($20.51 including two exploratory benchmarks) exceeds the committed $20 API credit cap by $0.51 due to cost variance during MultiTurn-Persona execution. Cost composition: $16.55 for the three pre-registered benchmarks (Skill-Retention $1.65, Persona-Align $1.77, OOS-Replay $13.13), $3.96 for two exploratory benchmarks (Mem-Utility $1.39, MultiTurn-Persona $2.57). The total remains within the originally budgeted $150–200 envelope by an order of magnitude. Actual API billing may differ slightly due to prompt caching adjustments. Underlying benchmark results (directional accuracy, skill inclusion, persona divergence) are unaffected — only cost figures changed.
...
#### Cycle 7: Process Slip Detection

**What was built.** Two exploratory benchmarks (Mem-Utility, MultiTurn-Persona) executed after an explicit prior commitment to stop at three pre-registered benchmarks and move to synthesis.

**What measurement caught.** External review identified three discrepancies: (a) the exploratory benchmarks were presented with confirmatory-grade framing ("suggestive," "motivates v0.2") despite lacking pre-registration and statistical power (n=25 and n=7 respectively); (b) the total cost of $20.51 exceeded the committed $20 API credit cap by $0.51 without disclosure; (c) subcategory results at n=5 were reported as carrying directional signal despite extremely wide confidence intervals.

**What changed.** All eight published artifacts were corrected within 24 hours: framing separated the three pre-registered benchmarks at planned scale from the two exploratory benchmarks marked as such, "suggestive" language was removed, the budget overrun was disclosed explicitly, and both JSON result files were annotated with `"protocol": "exploratory_post_hoc_not_pre_registered"` to prevent out-of-context citation.

**What re-measurement showed.** Zero remaining instances of overclaim language across all `.md`, `.html`, and `.py` files. All cost sections now disclose the $0.51 overrun. The exploratory benchmarks are clearly labeled in every artifact where they appear.

This cycle demonstrates the methodology applied to its own creator: discipline slips are caught and corrected by the same audit pattern applied to code and data. The slip's cause was velocity exceeding the rate of methodological consideration — a common failure mode in long projects where incremental work accumulates faster than the discipline that governs it.
...
**Exploratory benchmarks underpowered.** Mem-Utility (n=25) and MultiTurn-Persona (n=7) were executed without pre-registration and at reduced sample sizes due to budget constraints. Neither has the statistical power for confirmatory inference. The Mem-Utility null is the only interpretable finding at its sample size. The MultiTurn-Persona positive direction (+0.173) cannot be distinguished from noise at n=7. These results are reported for transparency; confirmatory runs with pre-registered protocols are scoped for v1.2.
...
**v1.2: Confirmatory multi-turn persona test.** The exploratory MultiTurn-Persona run (p=0.109, n=7) was underpowered and not pre-registered. A confirmatory run at n≥20 sequences with a pre-registered protocol is needed to determine whether persona convergence produces detectable behavioral change across turns. Estimated cost: $7–10.
...
Three pre-registered benchmarks were executed at planned scale with real agent calls and re-verified across two versions (v1.0 and v1.1). The planner achieves 81.7% skill-inclusion accuracy (5.2× corrected random baseline), demonstrating strong orchestration quality that is robust to data-availability fixes. The OOS Replay benchmark produced directional accuracy statistically indistinguishable from random (54.4% full-year, 50.0% same-period v1.1), confirming that the agent orchestrates well but the underlying strategy lacks sufficient edge for the orchestration to translate into directional alpha — a finding that cleanly separates orchestration quality from strategy alpha as independent evaluation dimensions. The persona-align benchmark confirmed a pre-registered null result with high confidence (p=0.70): in-prompt persona injection does not produce detectable behavioral change at v0.1, motivating v0.2 activation steering as the architectural path forward. Two exploratory benchmarks were executed at reduced scale due to budget constraints (Mem-Utility n=25, MultiTurn-Persona n=7); both are statistically underpowered and reported for transparency, with confirmatory runs scoped for v1.2.

The evaluation methodology contributes three integrity mechanisms: (1) OOS holdout enforcement via static AST analysis integrated into CI, which eliminates training data leakage at the source code level; (2) causal memory filtering via `query_ts` parameters threaded through all 8 retrieval paths, which prevents the agent from accessing future information during OOS replay; and (3) a self-audit cycle that revolved seven times, identifying and correcting seven distinct categories of integrity failure — OOS leakage, model provenance, measurement bugs, planner stop-conditions, cost tracking, architecture drift, and process slip (the methodology applied to its own author) — each documented with preserved audit trails, transparent disclosures, and re-verification. Total evaluation cost across all v1.1 runs was $20.51 ($0.51 over the committed $20 API credit cap) — demonstrating that continuous agentic evaluation is operationally feasible.
