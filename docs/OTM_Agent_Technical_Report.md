...
8. **An 8-benchmark evaluation suite** with domain-specific tests for long-context utilization, skill retention, persona alignment, distribution shift robustness, out-of-sample replay, memory utility, multi-turn persona consistency, and memory tier ablation. Three benchmarks executed and re-verified across two versions (v1.0, v1.1) with transparent audit trail: 81.7% skill-inclusion accuracy (5.1× baseline), pre-registered persona null confirmed (p=0.70), and directional accuracy statistically indistinguishable from random (54.4% full-year, CI crosses 50%). Six audit cycles documented with preserved trails.
...
The system includes 256 unit tests across 12 test modules, all passing:
...
The corrected total ($16.55) is still **9× under** the $150 pre-execution budget. Actual API billing may differ slightly due to prompt caching adjustments. Underlying benchmark results (directional accuracy, skill inclusion, persona divergence) are unaffected — only cost figures changed.
...
**What re-measurement showed.** v1.1 corrected total: $16.55 (30× the original figure). Still 9× under $150 budget. Cost amendment published to live site before v1.1 re-runs.
...
**Optional benchmarks deferred.** MultiTurn-Persona (multi-turn persona convergence) and Mem-Utility (memory ablation) were scoped for v1.1 but deferred to v1.2 due to budget constraints ($3.45 remaining of $20 credit). These benchmarks would directly test whether the persona Kalman update works across turns and whether the 4-tier memory adds measurable value — both important architectural questions for v0.2.
...
**Measure what you think you're measuring.** The cost tracking bug (§8.7) is an instance of a general anti-pattern: instrumenting one part of a multi-component system and treating its output as the system-level measurement. Skill costs ($0.55) looked plausible because they were real measurements — just of the wrong thing. The corrected figure ($16.55) emerged only after auditing all three LLM call sites independently. In agentic systems where multiple components contribute to cost, latency, or quality, every component must be instrumented and the system-level metric must aggregate all components. Silent fallback to partial measurement produces plausible-looking but wrong numbers.
...
**v1.2: Optional benchmarks.** MultiTurn-Persona (35–50 runs, testing persona Kalman convergence across turns) and Mem-Utility (100 runs, ablating memory system value) are the immediate priority. Estimated cost: $16–20. These benchmarks address the two largest open architectural questions: does persona conditioning work across turns (even if single-turn is null?), and does the 4-tier memory add measurable decision quality?
