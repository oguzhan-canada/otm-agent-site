...
8. **An 8-benchmark evaluation suite** with domain-specific tests for long-context utilization, skill retention, persona alignment, distribution shift robustness, out-of-sample replay, memory utility, multi-turn persona consistency, and memory tier ablation. Three pre-registered benchmarks executed at planned scale and re-verified across two versions (v1.0, v1.1): 81.7% skill-inclusion accuracy (5.1× baseline), pre-registered persona null confirmed (p=0.70), directional accuracy statistically indistinguishable from random (54.4% full-year, CI crosses 50%). Two exploratory benchmarks executed at reduced scale without pre-registration: memory utility null (−0.064, n=25, CI crosses zero) and multi-turn persona convergence underpowered (+0.173, p=0.109, n=7 — not interpretable at this sample size). Six audit cycles documented with preserved trails. Total evaluation cost: $20.51.
