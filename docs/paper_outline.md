...
| OTM-Mem-Utility | Is memory retrieval actually useful for decisions? | Ablation: with memory vs without. **v1.1 exploratory (not pre-registered, n=25 of 500): −0.064 (null, CI crosses zero, $1.39). Underpowered subcategories.** |
| OTM-MultiTurn-Persona | Persona consistency across multi-turn sessions | Skill divergence increases across turns. **v1.1 exploratory (not pre-registered, n=7 of 100): +0.173 convergence delta (p=0.109, $2.57). Underpowered — not interpretable.** |
